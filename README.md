...
- **Multilingual (en / vi / zh / ja)** — the entire UI language is driven by the **active course's `meta.locale`**: load a Vietnamese course and the whole app is Vietnamese, a Chinese course and it's Chinese, etc. Typed string dictionaries in `src/i18n/` (compiler-enforced completeness); `useT()` reads the active package's locale.
- **Accounts (sign up / sign in)** — static-appropriate auth behind a swappable `AuthProvider`: local accounts (username + Web-Crypto–hashed password, multiple profiles), **Continue with Google** (reuses the Drive integration), and a guest mode. Progress is **namespaced per account**; sign-out lives in Settings. A real backend can drop in by replacing the provider.
- **Data-driven courses**: all content (subjects, lessons, decks, shop, themes, achievements, stickers, games, branding, economy) comes from a swappable **Content Package** (one JSON file). Different package = different course, no code changes. Packages are AI-generatable, uploadable, and loadable from a static folder or Google Drive. Six courses ship (Vietnamese default + English, Chinese, Japanese, Space, and a 4-subject VN Grade-4 pack). See **[CONTENT_FORMAT.md](CONTENT_FORMAT.md)**.
- **Responsive** — full-screen on phones, centred device + persistent side-nav rail on tablet/desktop; distinctive rounded type (Baloo 2 + Be Vietnam Pro) with Vietnamese/CJK glyph support.
